...
Web pura (HTML + CSS + JavaScript, sin frameworks ni build), datos reales de la API del Banco Mundial y persistencia en Supabase con respaldo en `localStorage`. Autenticación con email/contraseña o con Google (OAuth vía Supabase).
...
└── js/
    ...
    │   └── supabase-data.js    # capa de datos (auth email + Google, perfiles, simulaciones)
...
Login con email/contraseña o con **Google** (OAuth gestionado por Supabase). Tras el redirect de Google, `index.html` captura la sesión y entra a la app. Sin Supabase configurado, la app cae a un flujo demo y guarda todo en `localStorage`.
